...
resource "google_network_connectivity_hub" "hub" {
  ...
  preset_topology = var.present_topology
}

resource "google_network_connectivity_group" "trusted_group" {
  count   = length(var.trusted_projects) > 0 ? 1 : 0
  project = var.project_id
  name    = var.name
  hub     = google_network_connectivity_hub.hub.id

  auto_accept {
    auto_accept_projects = var.trusted_projects
  }
}
...
resource "google_network_connectivity_spoke" "hybrid_spoke" {
  ...
  depends_on = [google_network_connectivity_hub.hub, google_network_connectivity_group.trusted_group]
}
